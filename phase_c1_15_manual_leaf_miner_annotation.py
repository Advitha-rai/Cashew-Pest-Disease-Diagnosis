...
Phase C.1.15 — Dedicated Single-Image Leaf_Miner Manual Annotation Tool
...
def resolve_and_validate_target(
    ...
    - Image exists in Dataset/Cleaned/Leaf miner
    - Class is Leaf_Miner (Code 2)
...
# ---------------------------------------------------------
def build_single_leaf_miner_ui(
    img_name: str,
    ...
    """
    Builds the dedicated single-image Leaf_Miner manual segmentation interface.
    ...
    html_code = f"""
<div id="leaf-miner-manual-container" style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 840px; margin: 15px auto; background: #FFFFFF; padding: 20px; border-radius: 10px; border: 2px solid #28A745; box-shadow: 0 4px 14px rgba(0,0,0,0.12);">
    ...
    <div style="margin-bottom: 14px; display: flex; flex-wrap: wrap; gap: 10px; align-items: center; background: #E9ECEF; padding: 10px 14px; border-radius: 6px;">
        <button onclick="setDrawingMode('brush')" id="btn-brush" style="background: #28A745; color: white; border: none; padding: 7px 16px; border-radius: 4px; font-weight: bold; cursor: pointer;">🖌️ Paint Leaf_Miner (Green)</button>
    ...
    <div style="margin-top: 16px; display: flex; justify-content: space-between; align-items: center; gap: 14px;">
        <div id="status-msg" style="font-weight: bold; color: #155724; font-size: 13px;">Ready: Paint visible leaf miner serpentine mines and click SAVE ANNOTATION.</div>
...
(function() {{
    ...
    window.saveAnnotation = function() {{
        ...
        if (window.google && google.colab && google.colab.kernel) {{
            google.colab.kernel.invokeFunction('colab_save_manual_leaf_miner_mask', [imageName, b64Data, manifestPath], {{}})
                .then(function(res) {{
                    btn.disabled = false;
...
# ---------------------------------------------------------
def colab_save_manual_leaf_miner_mask_handler(image_name: str, mask_base64: str, manifest_csv: str) -> Dict[str, Any]:
    """
    Saves and strictly validates the manually drawn Leaf_Miner mask.
    Enforces foreground value == 2, background == 0, uint8 dtype, mode L, size 224x224.
    ...
    # Convert foreground to Leaf_Miner code = 2
    ...
    backup_dir = manifest_path.parent / "backup_previous" / f"phase_c1_15_manual_leaf_miner_{timestamp_str}"
...
def register_colab_callbacks():
    ...
    try:
        ...
        output.register_callback("colab_save_manual_leaf_miner_mask", colab_save_manual_leaf_miner_mask_handler)
        return True
    except Exception:
        return False
...
def main():
    print("============================================================")
    print("PHASE C.1.15 — MANUAL LEAF_MINER SEGMENTATION TOOL")
    print("============================================================")
    ...
    if not TARGET_IMAGE:
        print("\n[ACTION REQUIRED]: Open phase_c1_15_manual_leaf_miner_annotation.py, enter TARGET_IMAGE, and rerun.")
        return
    ...
    try:
        ...
        ui_html = build_single_leaf_miner_ui(
